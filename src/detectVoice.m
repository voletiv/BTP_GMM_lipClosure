function [segments, fs, Limits, Eor, Cor, E,C] = detectVoice(soundFileName,fs,t)
...
if ischar(soundFileName)
    ...
    if (strcmpi(soundFileName(end-3:end),'.wav'))
        % read the wav file name:
        [x,fs] = wavread(soundFileName);
    else if  (strcmpi(soundFileName(end-3:end),'.mp3'))
        % read the wav file name:
        [x,fs] = mp3read(soundFileName);
    else
...
if (nargin==3) % plot results:
	%clf;
	figure, subplot(3,1,1); plot(Eor, 'g'); hold on; plot(E, 'c'); legend({'Short time energy (original)', 'Short time energy (filtered)'});
